...
ja.ui.scrollin = function (container, target, offset=0) {
  ja.log.dbg('ja.ui.scrollin()', 'container=', container, 'target=', target);
  var ul = $(container);
  if (!ul.length) return;
  var a = $(target);
  if (!a.length) return;
  ja.log.dbg('ja.ui.scrollin()', 'ul=', ul, 'a=', a);
  ul.scrollTop(ul.scrollTop() + a.position().top + offset)
}
